...
PRIVATE void	tty_write	(TTY* tty, char* buf, int len);
...
PUBLIC void task_tty()
{
	...
	while (1) {
		...
		case TTY_WRITE:
			/*
			 * all `printm()' will output chars to TTY_0
			 */
			tty_write(&tty_table[0],
				  (char*)va2la(src, msg.BUF),
				  msg.CNT);
			msg.type = SYSCALL_RET;
			send_recv(SEND, src, &msg);
			break;
 ...
 *****************************************************************************/
/**
 * The core routine invoked when `printm' is called.
 * 
 * @param tty  To which TTY chars will be output.
 * @param buf  The linear address of the buffer to be printed.
...
PUBLIC int sys_printx(int _unused1, int _unused2, char* s, struct proc* p_proc)
{
	 ...
	 */
	if (from_panic ||
	    (from_assert && proc_ready < &proc_table[NR_TASKS])) {
		disable_int();
...
#else
		if (from_assert) {
			for (ptty = TTY_FIRST + 1; ptty < TTY_END; ptty++)
				if (is_current_console(ptty->console))
					break;
			out_char(ptty->console, ch); /* output chars to current TTY */
		}
		out_char(TTY_FIRST->console, ch); /* always output chars to TTY 0 */
#endif
	}
